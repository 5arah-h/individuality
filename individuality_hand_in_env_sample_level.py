...
environment = env(n_joints=3,  # Number of joints
                  ...
                  # Maximal distance to grab the stick with the hand
                  stick_length=0.30,  # Stick length,
                  ball_size=0.10,
                  # Maximal distance to grab the ball with the stick
                  )
...
def compute_probability(data, mins, maxs, gs):
    ...
    return states / float(len(data[0]))
...
def compute_individuality(s_n_values, s__n_values, e_n_values, min_s, max_s, min_e, max_e):
    ...
    organismal = pid_data['SI'] + pid_data['UIY']  # mutual_information(p_s__n, p_s_n, p__s_s.flatten())
    colonial = pid_data['CI'] + pid_data['UIY']  # conditional_mutual_information(p_s__s_e, p_e_n, p_se, p__se)
    environmental_determined = pid_data['CI'] + pid_data['UIZ']  # conditional_mutual_information(p_e__s_s, p_s_n, p_es, p__s_s)
    env_coding = pid_data['SI'] - pid_data['CI']
    return organismal, colonial, environmental_determined, env_coding, pid_data


history_env = Dataset(3, 15)
history_env_future = Dataset(3, 15)
...
for step in range(iterations):
    ...
    if (step + 1) % intervall_measures == 0:
        ...
        e__ns = list(history_env_future.iter_y())  # e_n
    ...
    if noise:
        ...
        _, m_entropy = babbling_module.infer(babbling_module.expl_dims, babbling_module.inf_dims, babbling_module.x, n=1,
                                             explore=False)
        s_entropy = environment.update(m_entropy)

    m_first = [s_entropy[0], s_entropy[3], s_entropy[6]]
    m_middle = [s_entropy[1], s_entropy[4], s_entropy[7]]
    m_last = [s_entropy[2], s_entropy[5], s_entropy[8]]

    s_first = [s_entropy[9], s_entropy[12],
               s_entropy[15], s_entropy[18],
               s_entropy[21], s_entropy[24],
               s_entropy[27], s_entropy[30],
               s_entropy[33], s_entropy[36],
               s_entropy[39], s_entropy[42],
               s_entropy[45], s_entropy[48],
               s_entropy[51]]


    s_middle = [s_entropy[10], s_entropy[13],
                s_entropy[16], s_entropy[19],
                s_entropy[22], s_entropy[25],
                s_entropy[28], s_entropy[31],
                s_entropy[34], s_entropy[37],
                s_entropy[40], s_entropy[43],
                s_entropy[46], s_entropy[49],
                s_entropy[52]]


    s_last = [s_entropy[11], s_entropy[14],
              s_entropy[17], s_entropy[20],
              s_entropy[23], s_entropy[26],
              s_entropy[29], s_entropy[32],
              s_entropy[35], s_entropy[38],
    ...
    if m_first is not None:
        ...
        history_env_future.add_xy(m_last, s_last)
