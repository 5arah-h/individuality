...
from my_environment_and_joints import *
from learning_module import LearningModule, HierarchicalLearningModule,  HierarchicalLearningModuleCHC
...
def my_display_movement(fig, ax, environment, iterations=50):
	lines, = ax.plot([], [], lw=3)
	background = None
...
def random_motor_babbling(trial, iterations):
	env = My_Very_Own_Complex_Environment()
	...
	for iteration in range(iterations):
		...
		if (iteration + 1) % (iterations / 10) == 0:
			res += [
				int(compute_explo(array(explored_s)[:, [14, 17]], array([-2., -2.]), array([2., 2.]), gs=grid_size))]
	return res
...
def active_model_babbling(trial, iterations):
	...
	for step in range(iterations / (n_explore + 1)):
		...
		m = babbling_module.infer(babbling_module.expl_dims, babbling_module.inf_dims, babbling_module.x, n=1,
								  explore=False)
		s = env.update(m)  # execute this command and observe the corresponding sensory effect
