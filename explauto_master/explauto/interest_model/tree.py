...
class Tree(object):
    ...
    def sample_softmax(self, temperature=1.):
        ...
        if self.leafnode:
            ...
            if self.sampling_mode['volume']:
                progresses = map(lambda node_: node_.progress*node_.volume, nodes) #by volume
                progresses_list = list(progresses)

            else:
                progresses = map(lambda node_: node_.progress, nodes) #by volume
                progresses_list = list(progresses)


            progress_max = max(progresses_list)
            probas = np.exp(np.array(progresses_list) / (progress_max*temperature))
