...
def compute_individuality(s_n_values, s__n_values, e_n_values, min_s, max_s, min_e, max_e):
    ...
    box__n_values = np.array(s__n_values)[:,-1]
    s_rest__n_values = np.array(s__n_values)[:,:-1]

    box_n_values = np.array(s_n_values)[:,-1]
    s_rest_n_values = np.array(s_n_values)[:,:-1]

    box__n_values = [np.array([e]) for e in box__n_values]
    box_n_values = [np.array([e]) for e in box_n_values]

    p__s_s_e = compute_probability((box__n_values, s_rest__n_values, box_n_values, s_rest_n_values, e_n_values),
                                   (0., min_s, 0., min_s, min_e), (1., max_s, 1., max_s, max_e), (2, 3, 2, 3, 2))
    p__s_s_e = np.reshape(p__s_s_e, (27 * 2, 27 * 2, p__s_s_e.shape[-1]))
...
m_old = None
s_old = None
history_env = Dataset(4, 12)
history_system = Dataset(4, 4)
for step in range(iterations):
    ...
    m_first = [s_entropy[0], s_entropy[3], s_entropy[6], s_entropy[51]]
    m_middle = [s_entropy[1], s_entropy[4], s_entropy[7], s_entropy[52]]
    m_last = [s_entropy[2], s_entropy[5], s_entropy[8], s_entropy[53]]
