...
def compute_individuality(s_n_values, s__n_values, e_n_values, min_s, max_s, min_e, max_e):
    ...
    hand__n_values = np.array(s__n_values)[:,:2]
    s_rest__n_values = np.array(s__n_values)[:,2:]

    hand_n_values = np.array(s_n_values)[:,:2]
    s_rest_n_values = np.array(s_n_values)[:,2:]

    p__s_s_e = compute_probability((hand__n_values, s_rest__n_values, hand_n_values, s_rest_n_values, e_n_values),
...
for step in range(iterations):
    ...
    m_first = [s_entropy[9], s_entropy[12], s_entropy[0], s_entropy[3], s_entropy[6]]
    m_middle = [s_entropy[10], s_entropy[13], s_entropy[1], s_entropy[4], s_entropy[7]]
    m_last = [s_entropy[11], s_entropy[14], s_entropy[2], s_entropy[5], s_entropy[8]]
