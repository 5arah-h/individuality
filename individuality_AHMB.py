...
def get_dictionary(probabilities):
    return {(float(i), float(j), float(k)): float(probabilities[i][j][k]) for i, prob_i in enumerate(probabilities) for
            j, prob_j in enumerate(prob_i) for k, _ in enumerate(prob_j)}
...
def compute_probability(data, mins, maxs, gs):
    ...
    print('Probability distribution took in ms: ', time_diff)
...
for step in range(iterations):
    ...
    m_timestep = [s_entropy[2], s_entropy[5], s_entropy[8]]
    s_timestep = [s_entropy[17], s_entropy[20],
